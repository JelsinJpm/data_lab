    ...
    try:
        ...
        with open(csv_file, newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                subcategorias = eval(row['Subcategorías'])  # Convertir cadena a lista
                subcategoria_objs = []
                ...
                # Procesar colores
                colores = eval(row['Colores Disponibles'])  # Convertir cadena a lista
                color_objs = []
                ...
                # Procesar tallas
                tallas = eval(row['Tallas Disponibles'])  # Convertir cadena a lista
                talla_objs = []
                ...
                imagenes = eval(row['URLs de Imágenes'])  # Convertir cadena a lista
                for url in imagenes:
                    imagen = Imagen(producto_id=producto.producto_id, url=url)
                    session.add(imagen)
                
                # Procesar recomendaciones de cuidado
                recomendaciones = eval(row['Recomendaciones de Cuidado'])  # Convertir cadena a lista
...
insertar_datos_csv('C:/Users/johan/Desktop/Data/Caso uno/data/productos_scrapeados_v2_limpios.csv')